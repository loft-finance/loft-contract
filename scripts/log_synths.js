...
module.exports = async ({ getNamedAccounts, deployments, getChainId }) => {
  ...
  // console.log('totalIssuedSynths LOFT:', fromUnit((await read('Synthesizer', {}, 'totalIssuedSynths', toBytes32('LOFT'))).toString()));
  // console.log('totalIssuedSynths OfUSD:', fromUnit((await read('Synthesizer', {}, 'totalIssuedSynths', OfUSD)).toString()));
  ...
  await new Promise(async (resolve) => {
    accounts.forEach(async (account, index) => {
      // maxIssuableSynths
      const maxIssuableSynths = await read(
        "Synthesizer",
        {},
        "maxIssuableSynths",
        account
      ...
      console.log(
        `account${index + 1} maxIssuableSynths: ${fromUnit(
          maxIssuableSynths.toString()
        )}`
      );

      const transferableLoft = await read(
        "Synthesizer",
        {},
        "transferableLoft",
        account
      ...
      console.log(
        `account${index + 1} transferableLoft: ${fromUnit(
          transferableLoft.toString()
        )}`
      );
      ...
      console.log(
        `account${index + 1} collateralisationRatio: ${fromUnit(
          collateralisationRatio.toString()
        )}`
      );

      const balanceOf = await read(
        "Synthesizer",
        {},
        "debtBalanceOf",
        account,
      ...
      console.log(
        `account${index + 1}  debtBalanceOf: ${fromUnit(balanceOf.toString())}`
      );

      // lock value
      // ['LOFT', 'OfUSD', 'xAUD', 'xEUR'].forEach(async item => {
