...
contract("Synthesizer", async (accounts) => {
  ...
  const [ofUSD, xAUD, xEUR, LOFT, xBTC, xETH] = [
    "OfUSD",
    "xAUD",
    "xEUR",
    "LOFT",
  ...
  describe("adding and removing synths", () => {
    ...
    it("should disallow removing a Synth contract when it has an issued balance", async () => {
      ...
      // exchange to xAUD
      await synthesizer.exchange(ofUSD, toUnit("1"), xAUD, { from: owner });
  ...
  describe("totalIssuedSynths()", () => {
    it("should correctly calculate the total issued synths in a single currency", async () => {
      // Two people issue 10 ofUSD each. Assert that total issued value is 20 ofUSD.
      ...
      // Give some LOFT to account1 and account2
      await loft.transfer(account1, toUnit("1000"), { from: owner });
      await loft.transfer(account2, toUnit("1000"), { from: owner });

      // Issue 10 ofUSD each
      await synthesizer.issueSynths(toUnit("10"), { from: account1 });
      await synthesizer.issueSynths(toUnit("10"), { from: account2 });

      // Assert that there's 20 ofUSD of value in the system
      assert.bnEqual(await synthesizer.totalIssuedSynths(ofUSD), toUnit("20"));
    });

    it("should correctly calculate the total issued synths in multiple currencies", async () => {
      // Alice issues 10 ofUSD. Bob issues 20 xAUD. Assert that total issued value is 20 ofUSD, and 40 xAUD.
      ...
      await synthesizer.exchange(ofUSD, toUnit("20"), xAUD, { from: account2 });

      // Assert that there's 30 ofUSD of value in the system
      assert.bnEqual(await synthesizer.totalIssuedSynths(ofUSD), toUnit("30"));
    ...
    it("should return the correct value for the different quantity of total issued synths", async () => {
      ...
      await oracle.updateRates([xAUD, xEUR, LOFT], rates, timestamp, {
        from: oracleAccount,
      });
      ...
      // Give some LOFT to account1 and account2
      await loft.transfer(account1, toUnit("100000"), {
        from: owner,
      });
      await loft.transfer(account2, toUnit("100000"), {
        from: owner,
      });
      ...
      // Exchange
      await synthesizer.exchange(ofUSD, exchangeAmountToEUR, xEUR, {
        from: account1,
      });
      await synthesizer.exchange(ofUSD, exchangeAmountToEUR, xEUR, {
        from: account2,
      ...
      await synthesizer.exchange(ofUSD, exchangeAmountToAUD, xAUD, {
        from: account1,
      });
      await synthesizer.exchange(ofUSD, exchangeAmountToAUD, xAUD, {
        from: account2,
  ...
  describe("transfer()", () => {
    it("should transfer using the ERC20 transfer function", async () => {
      ...
      // e.g. owner owns all LOFT.

      assert.bnEqual(await loft.totalSupply(), await loft.balanceOf(owner));

      const transaction = await loft.transfer(account1, toUnit("10"), {
        from: owner,
      });
      ...
      assert.bnEqual(await loft.balanceOf(account1), toUnit("10"));
    });

    it("should revert when exceeding locked loft and calling the ERC20 transfer function", async () => {
      // Ensure our environment is set up correctly for our assumptions
      // e.g. owner owns all LOFT.
      assert.bnEqual(await loft.totalSupply(), await loft.balanceOf(owner));
      ...
      // Try to transfer 0.000000000000000001 LOFT
      await assert.revert(loft.transfer(account1, "1", { from: owner }));
    });

    it("should transfer using the ERC20 transferFrom function", async () => {
      // Ensure our environment is set up correctly for our assumptions
      // e.g. owner owns all LOFT.
      const previousOwnerBalance = await loft.balanceOf(owner);
      assert.bnEqual(await loft.totalSupply(), previousOwnerBalance);

      // Approve account1 to act on our behalf for 10 LOFT.
      let transaction = await loft.approve(account1, toUnit("10"), {
        from: owner,
      });
      ...
      transaction = await loft.transferFrom(owner, account2, toUnit("10"), {
        from: account1,
      });
      assert.eventEqual(transaction, "Transfer", {
        from: owner,
      ...
      // Assert that account2 has 10 LOFT and owner has 10 less LOFT
      assert.bnEqual(await loft.balanceOf(account2), toUnit("10"));
      assert.bnEqual(
        await loft.balanceOf(owner),
        previousOwnerBalance.sub(toUnit("10"))
      );

      // Assert that we can't transfer more even though there's a balance for owner.
      await assert.revert(
        loft.transferFrom(owner, account2, "1", {
          from: account1,
        })
      );
    });

    it("should revert when exceeding locked loft and calling the ERC20 transferFrom function", async () => {
      // Ensure our environment is set up correctly for our assumptions
      // e.g. owner owns all LOFT.
      assert.bnEqual(await loft.totalSupply(), await loft.balanceOf(owner));
      ...
      // Approve account1 to act on our behalf for 10 LOFT.
      const transaction = await loft.approve(account1, toUnit("10"), {
        from: owner,
      });
      assert.eventEqual(transaction, "Approval", {
        owner: owner,
      ...
      // Assert that transferFrom fails even for the smallest amount of LOFT.
      await assert.revert(
        loft.transferFrom(owner, account2, "1", {
          from: account1,
        })
    ...
    it("should not allow transfer if the exchange rate for loft is stale", async () => {
      // Give some LOFT to account1 & account2
      const value = toUnit("300");
      await loft.transfer(account1, toUnit("10000"), {
        from: owner,
      });
      ...
      await loft.transfer(account2, value, { from: account1 });

      await loft.approve(account3, value, { from: account2 });
      await loft.transferFrom(account2, account1, value, {
        from: account3,
      });
      ...
      await assert.revert(loft.transfer(account2, value, { from: account1 }));

      await loft.approve(account3, value, { from: account2 });
      await assert.revert(
        loft.transferFrom(account2, account1, value, {
          from: account3,
        })
      );
    });

    it("should not be possible to transfer locked loft", async () => {
      const issuedLOFT = web3.utils.toBN("200000");
      await loft.transfer(account1, toUnit(issuedLOFT), {
        from: owner,
      });
      ...
      await assert.revert(
        loft.transfer(account2, toUnit(issuedLOFT), {
          from: account1,
        })
      );
    });

    it("should lock newly received loft if the user's collaterisation is too high", async () => {
      ...
      await loft.transfer(account2, toUnit(issuedLOFT), {
        from: owner,
      });
      ...
      // Exchange into xEUR
      await synthesizer.exchange(ofUSD, maxIssuableSynths, xEUR, {
        from: account1,
      });
      ...
      // Increase the value of xEUR relative to loft
      const timestamp2 = await currentTime();
      await oracle.updateRates([xEUR], [toUnit("2.10")], timestamp2, {
        from: oracleAccount,
      });

      // Ensure that the new loft account1 receives cannot be transferred out.
      await loft.transfer(account1, toUnit("10000"), {
        from: account2,
      });
      await assert.revert(
        loft.transfer(account2, toUnit("10000"), { from: account1 })
      );
    });

    it("should unlock loft when collaterisation ratio changes", async () => {
      ...
      const transferable1 = await synthesizer.transferableLoft(account1);
      assert.bnEqual(transferable1, "0");

      // Exchange into xAUD
      await synthesizer.exchange(ofUSD, issuedSynths, xAUD, { from: account1 });
      ...
      const transferable2 = await synthesizer.transferableLoft(account1);
  ...
  describe("debtBalance()", () => {
    it("should not change debt balance % if exchange rates change", async () => {
      ...
      await loft.transfer(account1, toUnit("20000"), {
        from: owner,
      });
      await loft.transfer(account2, toUnit("20000"), {
        from: owner,
      });
      ...
      await synthesizer.exchange(ofUSD, amountIssuedAcc2, xAUD, {
        from: account2,
      });

      const PRECISE_UNIT = web3.utils.toWei(web3.utils.toBN("1"), "gether");
      let totalIssuedSynthofUSD = await synthesizer.totalIssuedSynths(ofUSD);
      const account1DebtRatio = divideDecimal(
        amountIssuedAcc1,
        totalIssuedSynthofUSD,
        PRECISE_UNIT
      );
      ...
      totalIssuedSynthofUSD = await synthesizer.totalIssuedSynths(ofUSD);
      const conversionFactor = web3.utils.toBN(1000000000);
      const expectedDebtAccount1 = multiplyDecimal(
        account1DebtRatio,
        totalIssuedSynthofUSD.mul(conversionFactor),
        PRECISE_UNIT
      ...
      assert.bnClose(
        await synthesizer.debtBalanceOf(account1, ofUSD),
        expectedDebtAccount1
      );
      assert.bnClose(
        await synthesizer.debtBalanceOf(account2, ofUSD),
        expectedDebtAccount2
      );
    });

    it("should correctly calculate a user's debt balance without prior issuance", async () => {
      await loft.transfer(account1, toUnit("200000"), {
        from: owner,
      });
      await loft.transfer(account2, toUnit("10000"), {
        from: owner,
      ...
      const debt1 = await synthesizer.debtBalanceOf(account1, ofUSD);
      const debt2 = await synthesizer.debtBalanceOf(account2, ofUSD);
    ...
    it("should correctly calculate a user's debt balance with prior issuance", async () => {
      ...
      const debt = await synthesizer.debtBalanceOf(account1, ofUSD);
  ...
  describe("maxIssuableSynths()", () => {
    it("should correctly calculate a user's maximum issuable synths without prior issuance", async () => {
      const rate = await oracle.rateForCurrency(toBytes32("LOFT"));
      const issuedLOFT = web3.utils.toBN("200000");
      await loft.transfer(account1, toUnit(issuedLOFT), {
        from: owner,
      });
    ...
    it("should correctly calculate a user's maximum issuable synths without any LOFT", async () => {
      const maxIssuableSynths = await synthesizer.maxIssuableSynths(account1);
    ...
    it("should correctly calculate a user's maximum issuable synths with prior issuance", async () => {
      const dows2usdRate = await oracle.rateForCurrency(LOFT);

      const issuedLOFT = web3.utils.toBN("320001");
      await loft.transfer(account1, toUnit(issuedLOFT), {
        from: owner,
      });
    ...
    it("should error when calculating maximum issuance when the LOFT rate is stale", async () => {
  ...
  describe("remainingIssuableSynths()", () => {
    it("should correctly calculate a user's remaining issuable synths with prior issuance", async () => {
      ...
      const issuedLOFT = web3.utils.toBN("200012");
      await loft.transfer(account1, toUnit(issuedLOFT), {
        from: owner,
      });
    ...
    it("should correctly calculate a user's remaining issuable synths without prior issuance", async () => {
      ...
      const issuedLOFT = web3.utils.toBN("20");
      await loft.transfer(account1, toUnit(issuedLOFT), {
        from: owner,
      });
  ...
  it("should correctly report hasIssued for an address", async () => {
    ...
    await synthesizer.issueMaxSynths({ from: owner });
    const synthBalance = await ofUSDContract.balanceOf(owner);
  ...
  it("should allow an issuer to issue synths in one flavour", async () => {
    ...
    // There should be 10 ofUSD of value in the system
    assert.bnEqual(await synthesizer.totalIssuedSynths(ofUSD), toUnit("10"));
  ...
  it("should allow two issuers to issue synths in one flavour", async () => {
    ...
    // There should be 30ofUSD of value in the system
    assert.bnEqual(await synthesizer.totalIssuedSynths(ofUSD), toUnit("30"));
  ...
  it("should allow multi-issuance in one flavour", async () => {
    ...
    // There should be 40 ofUSD of value in the system
    assert.bnEqual(await synthesizer.totalIssuedSynths(ofUSD), toUnit("40"));
  ...
  it("should allow an issuer to issue max synths via the standard issue call", async () => {
    ...
    // There should be 200 ofUSD of value in the system
    assert.bnEqual(await synthesizer.totalIssuedSynths(ofUSD), toUnit("200"));
  ...
  it("should disallow an issuer from issuing synths beyond their remainingIssuableSynths", async () => {
    ...
    // They should now be able to issue ofUSD
  ...
  it("should allow an issuer with outstanding debt to burn synths and decrease debt", async () => {
    ...
    // account1 should now have 200 ofUSD of debt.
    assert.bnEqual(
      await synthesizer.debtBalanceOf(account1, ofUSD),
      toUnit("200")
    );
    ...
    // account1 should now have 100 ofUSD of debt.
    assert.bnEqual(
      await synthesizer.debtBalanceOf(account1, ofUSD),
      toUnit("100")
    );
  ...
  it("should disallow an issuer without outstanding debt from burning synths", async () => {
    ...
    await ofUSDContract.transfer(account2, toUnit("100"), {
      from: account1,
    });
    await assert.revert(
      synthesizer.burnSynths(toUnit("10"), { from: account2 })
  ...
  it("should burn 0 when trying to burn synths that do not exist", async () => {
    ...
    await ofUSDContract.transfer(account2, toUnit("200"), {
      from: account1,
    });

    const debtBefore = await synthesizer.debtBalanceOf(account1, ofUSD);
    assert.ok(!debtBefore.isNeg());
    // Burning any amount of ofUSD will reduce the amount down to the current supply, which is 0
    await assert.revert(synthesizer.burnSynths("1", { from: account1 }));
    const debtAfter = await synthesizer.debtBalanceOf(account1, ofUSD);
  ...
  it("should only burn up to a user's actual debt level", async () => {
    ...
    await ofUSDContract.transfer(account1, toUnit("200"), {
      from: account2,
    });
    ...
    const balanceOfAccount1 = await ofUSDContract.balanceOf(account1);
    ...
    const balanceOfAccount1AfterBurn = await ofUSDContract.balanceOf(account1);
  ...
  it("should correctly calculate debt in a multi-issuance scenario", async () => {
    ...
    await loft.transfer(account2, toUnit("200000"), {
      from: owner,
    });
  ...
  it("should correctly calculate debt in a multi-issuance multi-burn scenario", async () => {
    // Give some LOFT to account1
    await loft.transfer(account1, toUnit("500000"), {
      from: owner,
    });
    await loft.transfer(account2, toUnit("14000"), {
      from: owner,
    });
  ...
  it("should allow me to burn all synths I've issued when there are other issuers", async () => {
    const totalSupply = await loft.totalSupply();
    const account2LOFT = toUnit("120000");
    const account1LOFT = totalSupply.sub(account2LOFT);

    await loft.transfer(account1, account1LOFT, {
      from: owner,
    }); // Issue the massive majority to account1
    await loft.transfer(account2, account2LOFT, {
      from: owner,
    ...
    await synthesizer.issueMaxSynths({ from: account1 });
    const debtBalance1 = await synthesizer.debtBalanceOf(account1, ofUSD);
    ...
    let debt = await synthesizer.debtBalanceOf(account2, ofUSD);
    await synthesizer.burnSynths(toUnit("43"), { from: account2 });
    debt = await synthesizer.debtBalanceOf(account2, ofUSD);
  ...
  describe("multiple issue and burn scenarios", () => {
    it("should correctly calculate debt in a high issuance and burn scenario", async () => {
      ...
      const debtBalance = await synthesizer.debtBalanceOf(account2, ofUSD);
    ...
    it("should correctly calculate debt in a high (random) issuance and burn scenario", async () => {
      ...
      for (let i = 0; i < totalTimesToIssue; i++) {
        ...
        // const db = await synthesizer.debtBalanceOf(account2, ofUSD);
    ...
    it("should correctly calculate debt in a high volume contrast issuance and burn scenario", async () => {
      ...
      // Give only 100 LOFT to account2
      const account2LOFT = toUnit("100");
      ...
      const debtBalance2 = await synthesizer.debtBalanceOf(account2, ofUSD);
  ...
  it("should prevent more issuance if the user's collaterisation changes to be insufficient", async () => {
    ...
    // exchange into xEUR
    await synthesizer.exchange(ofUSD, issuedSynths, xEUR, { from: account1 });
  ...
  it("Any user can check the collaterisation ratio for a user", async () => {
    const issuedLOFT = web3.utils.toBN("320000");
    await loft.transfer(account1, toUnit(issuedLOFT), {
      from: owner,
    });
  ...
  it("should be able to read collaterisation ratio for a user with loft but no debt", async () => {
    const issuedLOFT = web3.utils.toBN("30000");
    await loft.transfer(account1, toUnit(issuedLOFT), {
      from: owner,
    });
  ...
  it("should permit anyone checking another user's collateral", async () => {
    ...
    await loft.transfer(account1, amount, { from: owner });
  ...
  it("should calculate a user's remaining issuable synths", async () => {
    const transferredLOFT = toUnit("60000");
    await loft.transfer(account1, transferredLOFT, {
      from: owner,
    });
  ...
  it("should successfully burn all user's synths", async () => {
    ...
    await oracle.updateRates([LOFT], [toUnit("0.1")], timestamp, {
      from: oracleAccount,
    });
    ...
    // Then try to burn them all. Only 10 synths (and fees) should be gone.
    await synthesizer.burnSynths(await ofUSDContract.balanceOf(account1), {
      from: account1,
    });
    assert.bnEqual(await ofUSDContract.balanceOf(account1), web3.utils.toBN(0));
  });

  it("should burn the correct amount of synths", async () => {
    // Give some LOFT to account1
    await loft.transfer(account1, toUnit("400000"), {
      from: owner,
    });
    ...
    assert.bnEqual(await ofUSDContract.balanceOf(account1), toUnit("3000"));
  });
  ...
  it("should successfully burn all user's synths even with transfer", async () => {
    ...
    // Give some LOFT to account1
    await loft.transfer(account1, toUnit("300000"), {
      from: owner,
    });
    ...
    await ofUSDContract.transfer(account2, amountToTransfer, {
      from: account1,
    });
    const remainingAfterTransfer = await ofUSDContract.balanceOf(account1);
    await ofUSDContract.transfer(
      account1,
      await ofUSDContract.balanceOf(account2),
      {
        from: account2,
      }
    );
    ...
    assert.bnEqual(
      await ofUSDContract.balanceOf(account1),
  ...
  it("should allow the last user in the system to burn all their synths to release their synthesizer", async () => {
    ...
    await loft.transfer(account2, toUnit("140000"), {
      from: owner,
    });
    await loft.transfer(account3, toUnit("1400000"), {
      from: owner,
    });
    ...
    const debtBalance1After = await synthesizer.debtBalanceOf(account1, ofUSD);
    const debtBalance2After = await synthesizer.debtBalanceOf(account2, ofUSD);
    const debtBalance3After = await synthesizer.debtBalanceOf(account3, ofUSD);
  ...
  it("should allow a user to burn up to their balance if they try too burn too much", async () => {
    ...
    const debtBalanceAfter = await synthesizer.debtBalanceOf(account1, ofUSD);
  ...
  it("should allow users to burn their debt and adjust the debtBalanceOf correctly for remaining users", async () => {
    // Give some LOFT to account1
    await loft.transfer(account1, toUnit("4000000"), {
      from: owner,
    });
    await loft.transfer(account2, toUnit("4000000"), {
      from: owner,
    });
    ...
    let debtBalance1After = await synthesizer.debtBalanceOf(account1, ofUSD);
    let debtBalance2After = await synthesizer.debtBalanceOf(account2, ofUSD);
    ...
    debtBalance1After = await synthesizer.debtBalanceOf(account1, ofUSD);
    debtBalance2After = await synthesizer.debtBalanceOf(account2, ofUSD);
  ...
  it("should include escrowed reward loft when calculating a user's collaterisation ratio", async () => {
    const dows2usdRate = await oracle.rateForCurrency(LOFT);
    const transferredLOFT = toUnit("60000");
    await loft.transfer(account1, transferredLOFT, {
      from: owner,
    ...
    const escrowedLOFT = toUnit("30000");
    await loft.transfer(rewardEscrow.address, escrowedLOFT, {
      from: owner,
    });
    await rewardEscrow.appendVestingEntry(account1, escrowedLOFT, {
      from: account2,
    });
    ...
    const expectedCollaterisationRatio = divideDecimal(
      maxIssuable,
      multiplyDecimal(escrowedLOFT.add(transferredLOFT), dows2usdRate)
    );
    assert.bnEqual(collaterisationRatio, expectedCollaterisationRatio);
  });

  it("should permit user to issue ofUSD debt with only escrowed LOFT as collateral (no LOFT in wallet)", async () => {
    // Send a price update to guarantee we're not depending on values from outside this test.
    await oracle.updateRates(
      [xAUD, xEUR, LOFT],
      ["0.5", "1.25", "0.1"].map(toUnit),
    ...
    // ensure account1 has no LOFT balance
    const dowsBalance = await loft.balanceOf(account1);
    assert.bnEqual(dowsBalance, 0);

    // Append escrow amount to account1
    const escrowedAmount = toUnit("15000");
    await loft.transfer(rewardEscrow.address, escrowedAmount, {
      from: owner,
    });
    ...
    // Issue max synths. (300 ofUSD)
    await synthesizer.issueMaxSynths({ from: account1 });

    // There should be 300 ofUSD of value for account1
    assert.bnEqual(
      await synthesizer.debtBalanceOf(account1, ofUSD),
      toUnit("300")
    );
