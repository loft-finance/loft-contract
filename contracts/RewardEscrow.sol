...
/*
LOFT rewards are escrowed for 1 year from the claim date and users
    ...
    AddressResolverUpgradeable
{
     ...
     * These are the times at which each given quantity of LOFT vests. */
    mapping(address => uint256[2][]) public vestingSchedules;

    /* An account's total escrowed loft balance to save recomputing this for fee extraction purposes. */
    mapping(address => uint256) public totalEscrowedAccountBalance;

    /* An account's total vested reward loft. */
    mapping(address => uint256) public totalVestedAccountBalance;

    /* The total remaining escrowed balance, for verifying the actual loft balance of this contract against. */
    ...
    function setVestingScheduleTime(uint256 _vestingScheduleTime)
        public
        onlyOwner
    {
     ...
     * @return A pair of uints: (timestamp, loft quantity).
     */
    ...
    /**
     * @notice Get the quantity of LOFT associated with a given schedule entry.
     */
     ...
     * @dev A call to this should accompany a previous successful call to loft.transfer(rewardEscrow, amount),
     * to ensure that when the funds are withdrawn, there is enough balance.
     * Note; although this function could technically be used to produce unbounded
     * arrays, it's only withinn the 4 year period of the weekly inflation schedule.
     * @param account The account to append a new vesting entry to.
     * @param quantity The quantity of LOFT that will be escrowed.
     */
        ...
        onlyFeePool
    {
        ...
        require(
            totalEscrowedBalance <= loft().balanceOf(address(this)),
        ...
        if (scheduleLength == 0) {
            ...
            /* Disallow adding new vested LOFT earlier than the last one.
    ...
    /**
     * @notice Allow a user to withdraw any LOFT in their schedule that have vested.
     */
    function vest() external {
        ...
        if (total != 0) {
            ...
            loft().transfer(msg.sender, total);
