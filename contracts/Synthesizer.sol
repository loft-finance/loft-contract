    ...
    AddressResolverUpgradeable
{
    ...
    function removeSynth(bytes32 currencyKey) external onlyOwner {
        ...
        require(currencyKey != ofUSD, "Cannot remove ofUSD");
    ...
    function _issueSynthsFrom(address from, uint256 amount) internal {
        ...
        synths[ofUSD].issue(from, amount);
    ...
    function burnSynths(uint256 amount) external {
        ...
        uint256 existingDebt = debtBalanceOf(from, ofUSD);
        ...
        synths[ofUSD].burn(from, amountToBurn);
    ...
    {
        uint256 alreadyIssued = debtBalanceOf(_issuer, ofUSD);
    ...
    function maxIssuableSynths(address _issuer) public view returns (uint256) {
        // What is the value of their LOFT balance in the destination currency?
    ...
    function collateralisationRatio(address _issuer)
        ...
        returns (uint256)
    {
        uint256 totalOwnedLOFT = collateral(_issuer);
        if (totalOwnedLOFT == 0) return 0;

        uint256 debtBalance = debtBalanceOf(_issuer, "LOFT");
        return debtBalance.divideDecimalRound(totalOwnedLOFT);
    }

    function collateral(address account) public view returns (uint256) {
        uint256 balance = loft().balanceOf(account);
    ...
    function transferableLoft(address account)
        public
        view
        rateNotStale("LOFT")
        returns (uint256)
    {
        ...
        uint256 lockedLOFTValue = debtBalanceOf(account, "LOFT")
            .divideDecimalRound(issuanceRatio);

        if (lockedLOFTValue >= balance) {
            return 0;
        } else {
            return balance.sub(lockedLOFTValue);
        }
    }
    ...
    {
        // Ensure waitingPeriod and ofUSD balance is settled as burning impacts the size of debt pool
        ...
        // require liquidator has enough ofUSD
        require(
            IERC20(address(synths[ofUSD])).balanceOf(liquidator) >= susdAmount,
            "Not enough ofUSD"
        );
        ...
        // What is the value of their LOFT balance in ofUSD?
        ...
        // What is their debt in ofUSD?
        uint256 debtBalance = debtBalanceOf(account, ofUSD);
        ...
        // what's the equivalent amount of LOFT for the amountToLiquidate?
        ...
        // if total LOFT to redeem is greater than account's collateral
        // account is under collateralised, liquidate all collateral and reduce ofUSD to burn
    ...
    function _burnSynthsForLiquidation(
        ...
        ISynth(address(synths[ofUSD])).burn(burnForAddress, amountBurnt);

        // synth.burn does a safe subtraction on balance (so it will revert if there are not enough synths).
        ISynth(address(synths[ofUSD])).burn(liquidator, amount);
    ...
    function _addToDebtRegister(
        ...
        uint256 totalDebtIssued = totalIssuedSynths(ofUSD);
    ...
    function loft() internal view returns (ILoft) {
        return
            ILoft(
                resolver.requireAndGetAddress("LOFT", "Missing LOFT address")
            );
    }
