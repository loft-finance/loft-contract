    ...
    AddressResolverUpgradeable
{
    ...
    function recordFeePaid(uint256 amount) external onlyExchangerOrSynth {
        // Keep track off fees in ofUSD in the open fee pool period.
    ...
    function closeCurrentFeePeriod() external {
        require(
            _recentFeePeriodsStorage(0).startTime <=
                (block.timestamp - feePeriodDuration),
            "Too early to close fee period"
        );

        FeePeriod storage secondLastFeePeriod = _recentFeePeriodsStorage(
            FEE_PERIOD_LENGTH - 2
        );
        FeePeriod storage lastFeePeriod = _recentFeePeriodsStorage(
            FEE_PERIOD_LENGTH - 1
        );
        ...
        returns (uint256, uint256)
    {
        ...
        // And convert totalFees to ofUSD
        // Return totalRewards as is in LOFT amount
    ...
    function _applicableIssuanceData(address account, uint256 closingDebtIndex)
        ...
        returns (uint256, uint256)
    {
        IssuanceData[FEE_PERIOD_LENGTH]
            memory issuanceData = accountIssuanceLedger[account];
    ...
    function _effectiveDebtRatioForPeriod(
        ...
        // This is a high precision integer.
        uint256 feePeriodDebtOwnership = synthesizer()
    ...
    function _recordFeePayment(uint256 ofUSDAmount) internal returns (uint256) {
        // Don't assign to the parameter
        uint256 remainingToAllocate = ofUSDAmount;
    ...
    function _payFees(address account, uint256 ofUSDAmount)
        internal
        notFeeAddress(account)
    {
        ...
        // Grab the ofUSD Synth
        Synth ofUSDSynth = synthesizer().synths(ofUSD);
        ...
        ofUSDSynth.burn(FEE_ADDRESS, ofUSDAmount);

        // Mint their new synths
        ofUSDSynth.issue(account, ofUSDAmount);
    }
        ...
        notFeeAddress(account)
    {
        ...
        require(
            account != address(synthesizer()),
            "Can't send rewards to loft"
        );

        // Record vesting entry for claiming address and amount
        // LOFT already minted to rewardEscrow balance
        rewardEscrow().appendVestingEntry(account, dowsAmount);
    }

    /**
     * @dev onlyIssuer to call me on loft.issue() & loft.burn() calls to store the locked LOFT
    ...
    event FeesClaimed(address account, uint256 ofUSDAmount, uint256 dowsRewards);
}
