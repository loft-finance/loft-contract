...
contract Synth is Initializable, OwnableUpgradeable, ERC20Upgradeable, AddressResolverUpgradeable {
    ...
    function purge(address[] calldata addresses) external onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            ...
            if (amountHeld > 0) {
                exchanger().exchange(holder, currencyKey, amountHeld, ofUSD, holder);
    ...
    /**
     * non-ofUSD synths are exchanged into ofUSD via synthInitiatedExchange
     * notify feePool to record amount as fee paid to feePool 
    */
    function _transferToFeeAddress(address recipient, uint amount) internal{
        uint amountInUSD;

        if (currencyKey == ofUSD) {
            amountInUSD = amount;
            super._transfer(_msgSender(), recipient, amount);
        } else {
            amountInUSD = exchanger().exchange(_msgSender(), currencyKey, amount, ofUSD, feePool().FEE_ADDRESS());
        }
