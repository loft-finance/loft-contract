...
contract Exchanger is Initializable, AddressResolverUpgradeable {
    ...
    bytes32 private constant LOFT = "LOFT";
    ...
    function remitFee(
        ...
        // Remit the fee in ofUSDs
        uint256 usdFeeAmount = _oracle.effectiveValue(currencyKey, fee, ofUSD);
        _synthesizer.synths(ofUSD).issue(feePool().FEE_ADDRESS(), usdFeeAmount);
        // Tell the fee pool about this.
        feePool().recordFeePaid(usdFeeAmount);

        // Remit the rewards in LOFT
        uint256 rewardAmount = _oracle.effectiveValue(currencyKey, fee, LOFT);
